...
Board::Board(const Color& turn) : turn{turn}
{
...
bool Board::getCheckMate() const
{
	return checkMate;
}

bool Board::canPawnAttack(const auto& moveData, const std::array<std::array<Field, 8>, 8>& board)
{
...
bool Board::canPawnMove(const auto& moveData, const std::array<std::array<Field, 8>, 8>& board)
{
...
bool Board::isPawn(const auto& moveData, const std::array<std::array<Field, 8>, 8>& board)
{
...
bool Board::checkDirection(const auto& moveData, const std::array<std::array<Field, 8>, 8>& board)
{
...
bool Board::checkLength(const auto& moveData, const std::array<std::array<Field, 8>, 8>& board)
{
...
bool Board::checkModifier(const auto& moveData, const std::array<std::array<Field, 8>, 8>& board)
{
...
bool Board::checkIfFigureMoveIsCorrect(const auto& moveData, const std::array<std::array<Field, 8>, 8>& board)
{
	return checkDirection(moveData, board) and checkLength(moveData, board) and checkModifier(moveData, board);
}

bool Board::isSubscriptOutOfRange(const int x, const int y)
{
	return not(x >= 0 and y >= 0 and x <= 7 and y <= 7);
}

bool Board::checkForObstacles(const MoveData& ObstacleData, const std::array<std::array<Field, 8>, 8>& board)
{
	const Figure::Directions figureDirection{
	...
	{
	case Figure::Directions::vertical:
		if (n == 1 and !board[x + modX][y].isEmpty() and board[x][y].getFigureColor() == board[x + modX][y].
			getFigureColor())
		{
			return true;
		}
	...
	case Figure::Directions::horizontalAndVertical:
		if (n == 1)
		{
			if (!board[x + modX][y].isEmpty() and board[x][y].getFigureColor() == board[x + modX][y].
				getFigureColor())
			{
				return true;
			}
			if (!board[x][y + modY].isEmpty() and board[x][y].getFigureColor() == board[x][y + modY].
				getFigureColor())
			{
				return true;
			}
	...
	case Figure::Directions::diagonal:
		...
		if (n == 1 and !board[x + modX][y + modY].isEmpty() and board[x][y].getFigureColor() == board[x + modX][y +
				modY].
			getFigureColor())
		{
			return true;
	...
	case Figure::Directions::all:
		if (n == 1 and ((!board[x + modX][y].isEmpty() and board[x][y].getFigureColor() == board[x + modX][y].
			getFigureColor()) or (!board[x][y + modY].isEmpty() and board[x][y].getFigureColor() == board[x][y + modY].
			getFigureColor()) or (!board[x + modX][y + modY].isEmpty() and board[x][y].getFigureColor() == board[x +
				modX][y + modY].
			getFigureColor())))
		{
			return true;
	...
	case Figure::Directions::pawn:
		return canPawnMove(ObstacleData, board);
	case Figure::Directions::knight:
	default:
		return false;
	}
...
void Board::promote(const auto& moveData)
{
...
std::pair<Position, Position> Board::getKingPosition(const std::array<std::array<Field, 8>, 8>& board)
{
	Position whiteKing{};
	Position blackKing{};
...
bool Board::isKingOnPath(const int x, const int y, const std::array<std::array<Field, 8>, 8>& board)
{
...
bool Board::checkCheck(const std::array<std::array<Field, 8>, 8>& board)
{
	for (int x{0}; x < 8; ++x)
	{
		for (int y{0}; y < 8; ++y)
...
void Board::checkFieldsNearKing(const int x, const int y, std::map<Position, bool> kingMoveMap,
...
bool Board::canKingMove(const std::array<std::array<Field, 8>, 8>& board)
{
...
bool Board::isCheckingFigureOnPathOf(const int x, const int y, const std::array<std::array<Field, 8>, 8>& board)
{
	const Figure::Directions figureDirection{board[x][y].getFigureSchema().direction};
...
bool Board::canKillCheckingFigure(const std::array<std::array<Field, 8>, 8>& board)
{
	for (int x{0}; x < 8; ++x)
	{
		for (int y{0}; y < 8; ++y)
...
std::vector<Position> Board::getCheckingFigurePath(Position checkingFig,
                                                   const std::array<std::array<Field, 8>, 8>& board)
{
...
bool Board::canBlockPathOfCheckingFigure(const int x, const int y, Position checkingFig,
                                         const std::array<std::array<Field, 8>, 8>& board)
{
...
bool Board::canDefendKing(const std::array<std::array<Field, 8>, 8>& board)
{
	for (int x{0}; x < 8; ++x)
	{
		for (int y{0}; y < 8; ++y)
...
void Board::isCheckMate(const std::array<std::array<Field, 8>, 8>& board)
{
	if (checkCheck(board))
	{
...
bool Board::checkMoveDataForNullptr(const auto& moveData, const std::array<std::array<Field, 8>, 8>& board)
{
	return board[moveData.first.x][moveData.first.y].isEmpty();
}

bool Board::checkIfMoveIsPossible(const std::pair<Position, Position>& moveData)
{
	...
	if (!checkIfFigureMoveIsCorrect(moveData, fieldsCopy) or checkForObstacles(moveData, fieldsCopy))
	{
		return false;
	}
...
void Board::move(const std::pair<Position, Position>& moveData)
{
